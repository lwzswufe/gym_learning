...
class DQN():
    # DQN Agent
    def __init__(self, env, conv_1=[8, 4, 1], conv_2=[8, 3, 1], pool_1=[2, 2], pool_2=[2, 2]):
        ...
        self.Q_table = np.zeros([self.state_dim, self.action_dim], np.float32)
    ...
    def create_Q_network(self):
        ...
        # 原始数据输入范围是[-1, 1] 下一步会使用relu所以这里使用sigmod比较好
        ...
        # self.cnn_output = tf.layers.dense(flat, self.state_n)  # output layer
        ...
        self.Q_value = tf.matmul(self.cnn_output, self.Q_table)
        # q = W2 * h + b2
    ...
    def egreedy_action(self, state):  # e 贪婪策略
        cnn_output = self.cnn_output.eval(feed_dict={
            self.state_input: [state]})[0]
        Q_value = self.Q_value.eval(feed_dict={
            self.cnn_output: [cnn_output]
            })[0]
        if random.random() <= self.epsilon:  # 随机选择
            return random.randint(0, self.action_dim - 1), Q_value, cnn_output
        else:                                # 贪婪策略
            return np.argmax(Q_value), Q_value, cnn_output
...
def main():
    ...
    agent = DQN(env)
    Q_table = np.zeros([env.env.grid_num, len(env.env.actions)])

    action_past = -1
    cnn_stat_past = np.zeros([agent.state_dim, 1])
    q_value_past = np.zeros([1, agent.action_dim])
    ...
    for episode in range(EPISODE):
        ...
        for step in range(STEP):
            ...
            action, q_value, cnn_state = agent.egreedy_action(maze_now)
            ...
            if done:  # 终止状态
                reward = 1
                agent.Q_table[:, action] += cnn_state * reward
            else:
                reward = -0.05
                agent.Q_table[:, action] += cnn_state * reward

            if action_past >= 0:
                agent.Q_table[:, action_past] += cnn_stat_past * (max(q_value) - max(q_value_past))
                cnn_stat_past = cnn_state
                q_value_past = q_value
                if done:
                    action_past = -1
                else:
                    action_past = action
        ...
        if ((episode + 1) % 64 == 0) and (episode > BATCH_SIZE):
            ...
            print(context)
            context = ''
            for ii in range(len(cnn_state)):
                context += '|{:.2f} '.format(max(agent.Q_table[ii, :]))
            print(context)
